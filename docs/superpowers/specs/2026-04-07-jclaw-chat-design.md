...
布局分为四列：**项目栏**（最左侧窄边栏）+ **会话面板** + **聊天区域** + **iframe 业务面板**（默认隐藏，按需弹出）。
...
├── components/
│   ├── layout/
│   │   ├── ProjectSwitcher.vue  # 最左侧窄边栏：项目图标按钮列表（~60px 宽）
│   │   ├── SidePanel.vue        # 会话历史面板（含搜索框、按日期分组折叠/展开）
│   │   ├── ChatArea.vue         # 主聊天区域（iframe 隐藏时占满剩余宽度）
│   │   └── BusinessPanel.vue    # 右侧 iframe 容器 + postMessage 桥接（默认 hidden）
│   ├── chat/
...
1. 用户点击 `ProjectSwitcher` 中的项目图标按钮
...
├──┼──────────────┬──────────────────────────────────────────────────────────────────┤
│  │ 🔍 搜索      │                                                                  │
│G │              │  [用户气泡: 查看项目的动态利润分析]          →右对齐粉色          │
│  │ 今天      ▼  │                                                                  │
│S │  • 查看项目… │  [已完成思考 ▼]                                                  │
│  │              │                                                                  │
│S │ 本周      ▼  │  根据分析，项目目前预期利润为：2159.23万元…                      │
│  │  • 我要录入… │                                                                  │
│B │  • 我要录入… │  ┌──────────────────────────────────────────┐                   │
│  │              │  │ 材料  数量(吨)  单价  签收人  签收时间   │                   │
│  │ 本月      ▼  │  │ 螺纹钢   15    10    张三  2026-03-24   │                   │
│  │  • 我要录入… │  └──────────────────────────────────────────┘                   │
│  │              │                                                                  │
│  │ 2月       ▶  │                                                                  │
│  │              │                                                                  │
...
└──┴──────────────┴──────────────────────────────────────────────────────────────────┘
 ↑     ↑                          ↑
项目栏 会话面板(~270px)           聊天区域（iframe隐藏时占满）
(~60px)
```

**iframe 展开后（AI 返回含 open_modal 指令时）：**

```
┌──┬──────────────┬────────────────────────────────────┬──────────────────────────────┐
│  │ 会话面板     │  聊天区域（缩窄）                  │ iframe 业务面板          [×] │
│  │              │                                    │                              │
│  │              │  ...消息内容...                    │  *标题: [请输入标题      ]   │
│  │              │                                    │  *事故日期: [2024/08/01 📅] │
│  │              │  [🔖 新增质量隐患排查]              │  *内容: [请输入内容      ]   │
...
**iframe 面板行为规则：**
- 默认隐藏，聊天区域占满主区域宽度
- 仅当 AI 返回含 `action: "open_modal"` 的 JSON 时自动展开
- 展开后右上角显示 `[×]` 关闭按钮，用户可手动收起
- 关闭后聊天区域重新占满宽度
...
- 自动展开右侧 `BusinessPanel`（若已收起）
- 调用 `useIframeBridge.openModal(modal, data)`
- 状态条显示"JClaw 正在与业务界面交互中…"

**模式二 — 手动触发（操作卡片）：**
- AI 消息渲染 `ActionCard`，包含按钮（如"🔖 新增质量隐患排查"）
- 用户点击 → 展开 `BusinessPanel` → 调用 `useIframeBridge.openModal(modal, data)`
...
interface UseIframeBridge {
  ...
  isVisible: Ref<boolean>           // 控制 BusinessPanel 显隐
  openModal(modal: string, data: Record<string, unknown>): void  // 自动展开面板
  closePanel(): void                // 用户点击 [×] 收起面板
...
3. 发送文字消�� → 用户气泡出现，AI 回复流式渲染
4. AI 含思考内容 → "已完成思考"块出现，点击可展开
5. 点击左侧项目图标按钮 → 消息列表清空，新项目历史加载
6. 会话面板按时间分组展示会话，分组标题可折叠/展开；点击会话加载对应消息
7. 上传图片 → 消息气泡中显示缩略图
8. 底部四个快捷按钮可见并可点击
9. 断开 OpenClaw → 重连横幅出现并自动重���
10. 右侧 iframe 面板默认不可见，聊天区域占满主区域宽度
11. AI 返回含 `action: "open_modal"` 的 JSON → 右侧 iframe 面板自动展开并预填数据
12. 点击 ActionCard 中的"新增"按钮 → 右侧 iframe 面板展开弹窗
13. 右侧 iframe 面板展开时右上角显示 `[×]` 按钮，点击后面板收起
14. WS 连接成功后，底部显示 token 用量（inputTokens、outputTokens、费用、ctx 占比）
15. 每次 AI 回复结束后，用量数字自动刷新
16. ctx 使用率 > 80% 时，数字变橙色提示
17. 用户在 iframe 中保存 → JClaw 接收 postMessage，消息状态更新并显示记录链接